defmodule SpannexTest do
  ...
  test "can insert data into tables" do
    assert {:ok, _} = Spannex.transaction(Spannex.Conn, fn conn ->
      {:ok, _, [%{"id" => _}]} = Spannex.execute(
        conn,
  ...
  test "can read data within a formal transaction" do
    ...
    assert {:ok, {:ok, _, rows}} = Spannex.transaction(Spannex.Conn, fn conn ->
      Spannex.execute(
        conn,
        "SELECT * FROM foo",
        %{}
  ...
  test "supports named variables" do
    assert {:ok, {:ok, _, _}} = Spannex.transaction(Spannex.Conn, fn conn ->
      Spannex.execute(
        conn,
  ...
  test "supports multiple executions per transaction" do
    assert {:ok, _} = Spannex.transaction(Spannex.Conn, fn conn ->
      assert {:ok, _, [%{"id" => foo_id}]} = Spannex.execute(
        conn,
